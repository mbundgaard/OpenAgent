...
`CompletionEvent` is an abstract record with subtypes: `TextDelta`, `ToolCallEvent`, `ToolResultEvent`, `ThinkingStarted`, `ThinkingStopped`, `AssistantMessageSaved`. Both REST and WebSocket use the same events — REST collects them into a JSON array, WebSocket streams them as individual messages.

### Per-turn thinking events
`ThinkingStarted` and `ThinkingStopped` bracket tool execution per user turn. Both text providers yield `ThinkingStarted` once before the first tool-call round and `ThinkingStopped` once after all rounds complete — regardless of how many tools or rounds execute. Clients use these for waiting UX (spinners, thinking sounds). All transports use the same wire names: `thinking_started` / `thinking_stopped` — REST, WebSocket text, and WebSocket voice are unified. Voice sessions still emit per-tool `VoiceToolCallStarted`/`VoiceToolCallCompleted` internally; the WebSocket voice endpoint ref-counts them into per-turn `thinking_started`/`thinking_stopped`. Telnyx thinking pump is disabled pending re-enablement with per-turn events.
