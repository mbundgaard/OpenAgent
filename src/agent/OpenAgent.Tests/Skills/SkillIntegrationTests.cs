...
public class SkillIntegrationTests : IDisposable
{
    ...
    public async Task Full_lifecycle_activate_appears_in_prompt_deactivate_removed()
    {
        ...
        var prompt = promptBuilder.Build(conversation.Id, voice: false, conversation.ActiveSkills);
        ...
        conversation = _store.Get("conv1")!;
        var promptAfter = promptBuilder.Build(conversation.Id, voice: false, conversation.ActiveSkills);
